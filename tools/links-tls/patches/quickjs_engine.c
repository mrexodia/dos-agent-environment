...
static const char qjs_dom_bootstrap[] =
	...
	"				var tl = (t || \"\" ).toLowerCase();\n"
	"				if (tl === \"body\") return [dd.body || __body];\n"
	"				if (tl === \"head\") return [dd.head || __head];\n"
	"				if (tl === \"html\") return [dd.documentElement || __docel];\n"
	"				var r = __docel.getElementsByTagName(t);\n"
	"				return (r && r.length) ? r : [];\n"
	"			};\n"
	"		}\n"
	"	})();\n"
	...
	"		var __nativePromise = globalThis.Promise;\n"
	"		globalThis.queueMicrotask = function (f) {\n"
	"			/* MUST use the NATIVE promise captured now: core-js\n"
	"			 * replaces globalThis.Promise, and core-js notify calls\n"
	"			 * queueMicrotask -> mutual recursion amplified to 2GB\n"
	"			 * of retained strings (hn.algolia) */\n"
	"			__nativePromise.resolve().then(f);\n"
	"		};\n"
	"	}\n"
	"})();\n"
	"\n";
