...
int socket_and_bind(int pf, unsigned char *address)
{
	...
	{ extern long sock_open_count; extern void sock_log(const char *); sock_open_count++; sock_log("open"); }
#endif
	if (s == -1)
		return -1;
	if (address && *address) {
...
extern long sock_open_count, sock_close_count;
...
void make_connection(struct connection *c, int port, int *sock, void (*func)(struct connection *))
{
	...
	{ extern void sock_log2(const char *);
	  char mb[220];
	  snprintf(mb, sizeof mb, "MKCONN url=%.170s tries=%d", c->url, c->tries);
	  sock_log2(mb); }
#endif
	int socks_port = -1;
	unsigned char *host;
...
int get_pasv_socket(struct connection *c, int cc, int *sock, unsigned char *port)
{
	...
	{ extern void sock_log2(const char *);
	  char mb[200];
	  snprintf(mb, sizeof mb, "OPEN url=%.150s tries=%d", c->url, c->tries);
	  sock_log2(mb); }
#endif
	if (s == -1) goto e;
	*sock = s;
...
skip_numeric_address:
		...
		switch ((ret2 = SSL_get_error(c->ssl->ssl, ret1 = SSL_connect(c->ssl->ssl)))) {
			...
			ssl_error:
				log_ssl_error(c->url, __LINE__, ret1, ret2);
#ifdef WOLFSSL_VERSION
				{ extern void sock_log(const char *);
				  char mb[160];
				  sprintf(mb, "SSLFAIL url=%.80s ret1=%d ret2=%d err=%d errno=%d",
					  c->url, ret1, ret2, (int)ERR_get_error(), errno);
				  sock_log(mb); }
#endif
				ssl_downgrade_dance(c);
				return;
...
static void write_select(void *c_)
{
	...
	if (c->ssl) {
		...
		if ((wr = SSL_write(c->ssl->ssl, (void *)(wb->data + wb->pos), wb->len - wb->pos)) <= 0) {
			...
			{ extern void sock_log(const char *);
			  char mb[160];
			  sprintf(mb, "SSLWR-FAIL url=%.80s wr=%d err=%d werr=%d errno=%d",
				  c->url, wr, err, (int)ERR_get_error(), errno);
			  sock_log(mb); }
#endif
...
#ifdef WOLFSSL_VERSION
	{ extern void wolfssl_tlog(const char *); wolfssl_tlog("read_select\n"); }
#endif
read_more:
	...
	if (c->ssl) {
		if ((rd = SSL_read(c->ssl->ssl, (void *)(rb->data + rb->len), READ_SIZE)) <= 0) {
			...
			{ extern void sock_log(const char *);
			  char mb[160];
			  sprintf(mb, "SSLRD-FAIL url=%.80s rd=%d err=%d werr=%d errno=%d",
