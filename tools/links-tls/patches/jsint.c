...
void js_upcall_document_write(void *p, unsigned char *str, int len)
{
	...
	if (!js)return;
	if (!js->active) {
		/* post-load document.write (e.g. our form-submit search
		 * results rendered via the QuickJS fallback): append the
		 * text to the page source and re-render the frame. */
		if (!fd->f_data || !fd->rq) return;
		if (!js->src) {
			size_t len0;
			unsigned char *s0;
		...
		if ((unsigned)js->srclen + (unsigned)len > MAXINT) return;
		{
			unsigned char *s = mem_realloc(js->src, js->srclen + len);
			if (!s) return;
			memcpy(s + js->srclen, str, len);
			js->src = s;
			js->srclen += len;
			js->newdata += len;
			js_zaflaknuto_pameti += len;
