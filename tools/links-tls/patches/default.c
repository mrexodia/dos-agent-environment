...
#ifdef DOS
/* DOS networking is slow with too many connections. Keep 1 connection
 * per host: some servers (mojeek.com) drop burst connections
 * outright (TLS close -> SSL_ERROR_ZERO_RETURN). Serialized requests
 * are slower but reliable. */
int max_connections = 3;
int max_connections_to_host = 1;
#else
