...
unsigned char *get_form_url(struct session *ses, struct f_data_c *f, struct form_control *form, int *onsubmit)
{
	...
	if (!data) goto ff;
#ifdef JS
	if (form->method == FM_GET && f->js && f->js->ctx) {
		/* no inline onsubmit, but the page may have registered runtime
		 * submit listeners (jQuery addEventListener etc.). Dispatch a
		 * synthetic submit through QuickJS: site handlers run, and our
		 * fallback search renders visible results. preventDefault()
		 * cancels the native GET navigation. */
		extern int qjs_form_submit(void *, const char *);
		{
			char mb[192];
			extern void sock_log2(const char *);
			sprintf(mb, "FORMSUBMIT hook form=%d data=%.80s",
				form->form_num, (const char *)data);
			sock_log2(mb);
		}
		if (qjs_form_submit(f->js->ctx, (const char *)data)) {
			if (onsubmit) *onsubmit = 1;
			mem_free(data);
